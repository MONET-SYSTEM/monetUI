...
class _SetupPinScreenState extends State<SetupPinScreen> {
        ...
        return Padding(
          ...
          child: Container(
            ...
            decoration: BoxDecoration(
              border: Border.all(
                color: AppColours.primaryColourLight.withOpacity(0.5),
                width: 4,
              ),
              color: ((step == 1 ? _pinConfirmation.length : _pin.length) >=
                  index + 1)
                  ? Colors.white
                  : Colors.transparent,
            ...
            children: List.generate(3, (index) {
              ...
              return Padding(
                ...
                child: TextButton(
                  ...
                  child: Text(
                    number.toString(),
                    style: AppStyles.medium(size: 48, color: Colors.white),
                  ),
                ),
              );
            }),
  ...
  void _selectNumber(int number) {
    if (step == 0) {
      ...
      _pinConfirmation += number.toString();
    }
    setState(() {});
  }
  ...
  void _handleBackspace() {
    if (step == 0) {
      ...
      _pinConfirmation =
          _pinConfirmation.substring(0, _pinConfirmation.length - 1);
    }
    setState(() {});
  }
  ...
  Future<void> _handleSubmit() async {
    if (step == 0) {
      ...
      if (_pin != _pinConfirmation) {
        Helper.snackBar(
          context,
          message: AppStrings.pinsDoNotMatch,
          isSuccess: false,
        );
        setState(() {
          step = 0;
      ...
      // Submit PIN
      final result = await AuthController.setPin(_pin);
      // If widget is unmounted after async call, stop
      if (!mounted) return;
